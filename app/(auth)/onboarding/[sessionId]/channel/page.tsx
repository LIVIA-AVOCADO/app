...
import { WhatsappConnect } from '@/components/onboarding/channel/whatsapp-connect';
...
export default async function OnboardingChannelPage({ params }: PageProps) {
  ...
  if (session.payload?.channel?.connection_status === 'connected') {
    redirect(`/onboarding/${sessionId}/review`);
  }
  ...
  return (
    <div className="flex h-screen flex-col bg-zinc-50 dark:bg-zinc-950">
      ...
      <div className="flex items-center gap-3 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-6 py-4 shrink-0">
        ...
        <div>
          <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Canal WhatsApp</p>
          <p className="text-xs text-zinc-400">Conecte seu número</p>
        </div>
